...
程序描边系统在角色外围绘制 1 像素宽的黑色轮廓，并支持按层级规则控制描边的遮挡关系。

- **层级优先级函数 `GetLayerPriority(srcId)`**：
  - 集中定义了所有层的渲染优先级，描边逻辑只调用此函数，不在描边代码中重复写规则；
  - 优先级数值越大表示越在前面：
    - 背包 (`SRC_BAG`)：朝北时 100（最前），朝南时 0（最后）
    - 主手武器 (`SRC_WEAPON0`)：朝南时 80，朝北时 10
    - 副手武器 (`SRC_WEAPON1`)：朝南时 70，朝北时 5
    - 人本体及装备 (`SRC_MAIN/SRC_CLOAK/SRC_HELMET` 等)：50
    - 空 (`SRC_NONE`)：-1

- **描边判断逻辑**：
  - 只在**地面基线之上**的像素上考虑画描边；是否真正画描边，由各层在该像素的 **center alpha + 层级优先级** 决定：
  - 对 4 个相邻像素分别采样武器层、人本体层、背包层的 alpha：
    - **武器描边**：邻居有武器像素 + 当前像素不在头部核心区域 + 武器优先级 ≥ 当前像素优先级；
      - 额外检查：若邻居同时有角色本体或背包像素，且它们的优先级更高（例如朝北时身体/背包都在武器前），则不画武器描边，避免盾牌描边跑到人/背包外围；
    - **人本体描边**：邻居有本体像素 + 本体优先级 ≥ 当前像素优先级；
      - 额外检查：若邻居是手部区域且当前像素 `srcId` 是武器，不画手部描边，避免手部描边遮挡武器；
    - **背包描边**：邻居有背包像素 + 背包优先级 ≥ 当前像素优先级；
  - Shader 内会在武器/本体/背包三层中选取优先级最高的作为描边来源，若 `bestPriority >= 0` 则在该像素绘制 1 像素宽的轮廓。

- **描边颜色优先级**：
  - Hit 描边 (`_HitOutline`) > Extra 描边 (`_ExtraOutlineEnabled`) > 默认黑色
...
- **v2.6**: 程序描边优先级系统
  - 新增 `GetLayerPriority(srcId)` 函数，集中管理武器/人/背包的层级优先级规则；
  - 描边逻辑只通过此函数判断优先级，不再在描边代码中重复写渲染规则；
  - 手部描边不会遮挡武器：检测邻居是否为手部区域，若当前像素有武器颜色则跳过手部描边；
  - 朝北时武器描边不会出现在角色外围：检测邻居是否同时有角色像素，若角色优先级更高则跳过武器描边；
  - 描边只在地面基线之上绘制，并通过 center alpha + 层级优先级控制可画位置，避免与地面黑线等细节冲突。
